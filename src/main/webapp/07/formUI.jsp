...
<h4>훈련생 등록</h4>
<form action="${pageContext.request.contextPath }/07/formProcess" method="post" enctype="application/x-www-form-urlencoded">
<ul>
	<li>
	...
	<li>
		경력사항 :
		<textarea name="career">초기텍스트</textarea> 
	</li>
	<li>
	...
	<li>
		...
		<input type="checkbox" name="hobbies" value="H2"/>게임
		<input type="checkbox" name="hobbies" value="H3"/>게임
	</li>
	<li>
...
<pre>
	input : type(입력 데이터의 종류), name(parameter명, part명)
	...
	select : name(parameter명, part명)
			 disabled(전송 데이터에서 제외, boolean), required(필수입력 여부, boolean)
		   	 readonly(값의 변경 가능 여부, boolean)
		   	 option으로  value가 결정됨
		   	 option label(사용자에게 보여주는 텍스트), value(전송 데이터 값)
